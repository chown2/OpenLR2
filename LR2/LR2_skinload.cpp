			...
			if (*fBuf.atPos(0) == '#') {
				...
				if (!fBuf.left(1).isDiff("#")) {
					...
					case "#DST_NOWJUDGE_1P"_hash:{
						SplitCSV(fBuf, &csv, ",");
						if (CheckIndexRange(csv.val[1], 0, 5, line, pFbuf)) {
							ReadDST(&sk->dst_NOWJUDGE[PLAYER_1][csv.val[1]], &csv, tSkin_num);
					...
					case "#DST_NOWCOMBO_1P"_hash:{
						SplitCSV(fBuf, &csv, ",");
						ReadDST(&sk->dst_NOWCOMBO[PLAYER_1][csv.val[1]], &csv, tSkin_num);
					...
					case "#DST_NOWJUDGE_2P"_hash:{
						SplitCSV(fBuf, &csv, ",");
						if (CheckIndexRange(csv.val[1], 0, 5, line, pFbuf)) {
							ReadDST(&sk->dst_NOWJUDGE[PLAYER_2][csv.val[1]], &csv, tSkin_num);
					...
					case "#DST_NOWCOMBO_2P"_hash:{
						SplitCSV(fBuf, &csv, ",");
						if (CheckIndexRange(csv.val[1], 0, 5, line, pFbuf)) {
							ReadDST(&sk->dst_NOWCOMBO[PLAYER_2][csv.val[1]], &csv, tSkin_num);
					...
					case "#DST_GAUGECHART_1P"_hash:{
						SplitCSV(fBuf, &csv, ",");
						if (CheckIndexRange(csv.val[1], 0, 1, line, pFbuf)) {
							ReadDST(&sk->dst_GAUGECHART[PLAYER_1][csv.val[1]], &csv, tSkin_num);
						}
						break;
					}
					...
					case "#DST_GAUGECHART_2P"_hash:{
						SplitCSV(fBuf, &csv, ",");
						if (CheckIndexRange(csv.val[1], 0, 1, line, pFbuf)) {
							ReadDST(&sk->dst_GAUGECHART[PLAYER_2][csv.val[1]], &csv, tSkin_num);
						}
						break;
					}

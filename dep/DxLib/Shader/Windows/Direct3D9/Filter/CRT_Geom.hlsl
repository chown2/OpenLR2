float4 __inputSize : register(c0);
float4 __scale : register(c1);
...
int2 GetInputSize() { return int2(__inputSize.x, __inputSize.y); }
int2 GetOutputSize() { return int2(__inputSize.z, __inputSize.w); }
float2 GetScale() { return float2(__scale.x, __scale.y); }
...
#define TEX2D(c) pow(tex2D(sam, (c)), CRTGamma)
...
{
    const int2 outputSize = GetOutputSize();
    const int2 inputSize = GetInputSize();
